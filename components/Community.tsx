...
const Community: React.FC<CommunityProps> = ({ user, lang }) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] bg-slate-900">
      <div className="bg-slate-800 p-4 shadow-md z-10 flex justify-between items-center">
        <div>
        ...
        <div className="bg-slate-700 px-3 py-1 rounded-full text-xs text-slate-300 flex items-center gap-1">
      ...
      <div className="p-4 bg-slate-800 border-t border-slate-700 pb-24">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-900 border border-slate-600 rounded-full px-4 py-3 text-white focus:outline-none focus:border-emerald-500"
          />
          <button 
            onClick={handleSend}
            className="bg-emerald-600 p-3 rounded-full text-white hover:bg-emerald-500 transition-colors"
          >
            <Send size={20} />
          </button>
        </div>
